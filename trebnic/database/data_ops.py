...
class DataOpsMixin:
    ...
    async def export_all(self) -> Dict[str, Any]:
        """Export all data as a dict suitable for JSON serialization."""
        try:
            async with self._get_connection() as conn:
                projects = []
                async with conn.execute("SELECT id, name, icon, color FROM projects") as cur:
                    projects = [dict(r) async for r in cur]
                ...
                async with conn.execute(
                    ...
                    for r in await cur.fetchall():
                        row = dict(r)
                        weekdays = row.get("recurrence_weekdays", "[]")
                        if isinstance(weekdays, str):
                            row["recurrence_weekdays"] = json.loads(weekdays)
                        tasks.append(row)
                ...
                daily_notes = []
                async with conn.execute("SELECT date, content, updated_at FROM daily_notes") as cur:
                    daily_notes = [dict(r) async for r in cur]
