...
class TimerWidget(ft.Container): 
    def __init__(self, on_stop): 
        ...
        super().__init__( 
            content=ft.Column([ft.Row([ft.Icon(ft.Icons.TIMER), self.display], tight=True), 
                               self.task_text], tight=True, spacing=2, horizontal_alignment=ft.CrossAxisAlignment.CENTER), 
            bgcolor=COLORS["accent"], padding=ft.padding.symmetric(horizontal=15, vertical=5), 
            border_radius=20, visible=False, on_click=on_stop, ink=True, tooltip="Click to stop timer") 
