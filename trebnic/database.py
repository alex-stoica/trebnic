...
from constants import DEFAULT_ESTIMATED_SECONDS 

DB_PATH = Path("trebnic.db")

sqlite3.register_adapter(date, lambda d: d.isoformat())
sqlite3.register_converter(
    "DATE", lambda s: date.fromisoformat(s.decode()) if s else None 
)
...
class Database:
    ...
    def save_task(self, t: Dict) -> int:
        ...
        params = ( 
            t["title"], t["spent_seconds"], t["estimated_seconds"], t["project_id"], 
        ...
        if t.get("id") is None:
            cur = self.conn.execute( 
                ...
                "recurrence_weekdays,notes,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", 
                params 
            ) 
            self.conn.commit()
        ...
        self.conn.execute( 
            ...
            "due_date=?,is_done=?,recurrent=?,recurrence_interval=?,recurrence_frequency=?," 
            "recurrence_weekdays=?,notes=?,sort_order=? WHERE id=?", 
            params + (t["id"],) 
        ) 
        self.conn.commit()
        return t["id"]
    ...
    def load_tasks(self) -> List[Dict]:
        ...
        return [ 
            {**dict(r), "recurrence_weekdays": json.loads(r["recurrence_weekdays"])} 
            for r in rows 
        ] 
