...
class TaskService:
    ...
    @staticmethod
    def _init_defaults(state: AppState) -> None:
        defaults = [
    ...
    @staticmethod
    def _load_from_db(state: AppState) -> None:
    ...
    def duplicate_task(self, task: Task) -> Task:
        ...
        new_task = Task(
            ...
            recurrence_frequency=task.recurrence_frequency,
            recurrence_weekdays=(
                task.recurrence_weekdays.copy() if task.recurrence_weekdays else []
            ),
        )
        new_task.id = db.save_task(new_task.to_dict())
        self.state.tasks.append(new_task)
    ...
    def task_name_exists(self, name: str, exclude: Optional[Task] = None) -> bool:
        for t in self.state.tasks + self.state.done_tasks:
