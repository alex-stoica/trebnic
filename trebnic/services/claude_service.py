...
async def refine_note(content: str, instruction: str) -> str:
    """Refine a note using Claude. No tools, no TrebnicAPI dependency.
    ...
    system = (
        "You are a writing assistant. The user will give you a note and an instruction. "
        "Apply the instruction to the note and return ONLY the refined note text. "
        "Do not add any commentary, explanation, or meta-text. "
        "Preserve the original language and markdown formatting."
    )
...
class ClaudeService:
    ...
    async def _dispatch(
        ...
        if name == "complete_task":
            task = await _resolve_task(state, args["task_id"])
            if not task:
...
async def _resolve_task(state: AppState, task_id: int) -> Optional[Task]:
    """Find a task by ID from state, falling back to DB if not in memory."""
    task = state.get_task_by_id(task_id)
    if task:
        return task
    row = await db.load_task_by_id(task_id)
    if row:
